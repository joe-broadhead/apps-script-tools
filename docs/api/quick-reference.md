...
df.pivot(indexCol, pivotCol, aggMapping)
...
series.rolling(windowSize, operation)
series.expanding(operationOrOptions, options)
series.ewm(options)
...
- `ASTX.DBT.columnLineage(...)` infers column links across lineage edges with deterministic confidence scoring (`confidenceThreshold`, `maxMatchesPerEdge`).
...
- Storage `transfer/copyPrefix/sync` can bridge providers using read+write fallback when source and target providers differ.
