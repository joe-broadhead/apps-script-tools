...
function astRuntimeSerializeErrorCause_(cause) {
  ...
  if (typeof cause === 'object') {
    return astRuntimeCloneSerializableValue_(cause, []);
  }
  return { message: String(cause) };
}

function astRuntimeCloneSerializableValue_(value, seen) {
  if (value == null) {
    return value;
  }
  ...
  if (Array.isArray(value)) {
    ...
    for (let i = 0; i < value.length; i += 1) {
      outputArray.push(astRuntimeCloneSerializableValue_(value[i], seen));
    }
    seen.pop();
    return outputArray;
  }
  ...
  for (let i = 0; i < keys.length; i += 1) {
    ...
    output[key] = astRuntimeCloneSerializableValue_(entry, seen);
  }
  seen.pop();
  return output;
}
